...
class Fccp
{
	...
	public:  struct SnapshotDesc
	{
		...
		union
		{
			float FloatTriggerLevel;
			long  LongTriggerLevel;
		};
		float TriggerFilterTimeConstant;
	...
	{
		bool condition = *(char *)_snapshot.TrigAddress >= (_snapshot.LongTriggerLevel & 0xFF);
		bool trig = condition && !_snapshot.PrevCondition;
		_snapshot.PrevCondition = condition;
		return trig;
	}
	...
	{
		bool condition = *(short *)_snapshot.TrigAddress >= (_snapshot.LongTriggerLevel & 0xFFFF);
		bool trig = condition && !_snapshot.PrevCondition;
		_snapshot.PrevCondition = condition;
		return trig;
	}
	...
	{
		bool condition = *(long *)_snapshot.TrigAddress >= _snapshot.LongTriggerLevel;
		bool trig = condition && !_snapshot.PrevCondition;
		_snapshot.PrevCondition = condition;
		return trig;
	}
	...
	{
		bool condition = *(float *)_snapshot.TrigAddress >= _snapshot.FloatTriggerLevel;
		bool trig = condition && !_snapshot.PrevCondition;
		_snapshot.PrevCondition = condition;
		return trig;
	}

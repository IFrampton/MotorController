...
unsigned long DataMan::_dummyData[2] = {0xDEADBEEF, 0xDEADBEEF};
...
unsigned long DataMan::_dataLength = (unsigned long)&_variables.DigitalOut.BookEnd - (unsigned long)&_variables;
unsigned long DataMan::_configLength[2] =
{
	(unsigned long)&_config.Analog->BookEnd - (unsigned long)_config.Analog, (unsigned long)&_config.Digital->BookEnd - (unsigned long)_config.Digital
};
...
void *DataMan::GetConfigAddress(unsigned long address, bool relative, unsigned char module, unsigned char type)
{
	...
	if(relative)
	{
		if(type == 0)
		{
			...
			unsigned long offset = _relativeConfigLookup[module][1] + address;
			if(offset >= _configLength[1])
			{
				return (void *)_dummyData;
			}
			return (void *)((baseLocation + offset) & 0xFFFFFFFC);
		}
		unsigned long baseLocation = (unsigned long)_config.Analog;
		unsigned long offset =  _relativeConfigLookup[module][0] + address;
		if(offset >= _configLength[0])
		{
			return (void *)_dummyData;
		}
		return (void *)((baseLocation + offset) & 0xFFFFFFFC);
...
void *DataMan::GetVariableAddress(unsigned long address, unsigned char module, unsigned char type)
{
	...
	unsigned long offset = _relativeVariableLookup[module][type] + address;
